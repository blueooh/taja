...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      authUser = { id: existing.id, username: user.email ?? '', nickname: existing.nickname, isSocial: true }
    } else {
      const rawName =
      ...
      authUser = { id: newUser.id, username: user.email ?? '', nickname: newUser.nickname, isSocial: true }
    }
